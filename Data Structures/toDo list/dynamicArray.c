...
void dyHeapRemoveMin(DynamicArray* heap, compareFunction compare)
{
	...
	adjustHeap(heap, last, 0, compare); // perculate the value down into position to fix the heap ordering
}
...
TYPE dyHeapGetMin(DynamicArray* heap)
{
	assert(heap != 0);
	assert(dySize(heap) != 0);
	return heap->data[0]; // return the root's value
}
...
void dyHeapSort(DynamicArray* heap, compareFunction compare)
{
	...
	for (int i = dySize(heap) - 1; i >= 0; i--)
	{
		...
		adjustHeap(heap, i, 0, compare); // perculate the value down into position to fix the heap ordering
	}
}
