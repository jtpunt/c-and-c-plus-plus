...
int main(int argc, char *argv[]){
    ...
    double maxMegaBodies = 0.;
    double sumMegaBodies = 0.;
    ...
    for(int t = 0; t < NUMSTEPS; t++){
        #if PARALLELISMTYPE == 1 // When this is true, we are testing for Coarse-Grained Parallelism. THREADTYPE = static or dynamic
            #pragma omp parallel for default(none), shared(Bodies), schedule(THREADTYPE) 
        #endif
        for(int i = 0; i < NUMBODIES; i++){
            float fx = 0.;
            float fy = 0.;
            float fz = 0.;
            Body *bi = &Bodies[i];
            #if PARALLELISMTYPE == 2 // When this is true, we are testing for Fine-Grained Parallelism. SCHEDULETYPE = static or dynamic
                #pragma omp parallel for default(none), shared(Bodies, i, bi), reduction(+:fx,fy,fz), schedule(THREADTYPE) 
            #endif
    ...
    double megaBodies = ((float)(NUMBODIES*NUMBODIES*NUMSTEPS)/(time1-time0)/1000000.); // correct - according to piazza
    sumMegaBodies += megaBodies;
    if(megaBodies > maxMegaBodies){
        maxMegaBodies = megaBodies;
    }
    // printf("NUMTHREADS: %d, NUMNODES: %d, volume: %lf, maxMegaBodies: %lf, time: %lf\n", NUMTHREADS, NUMNODES, totalVolume, maxMegaBodies, elapsedTime);
    printf("maxMegaBodies: %lf, time: %lf\n\n",  maxMegaBodies, time1-time0);
    return 0;
}
