...
const char *WINDOWTITLE = { "OpenCL/OpenGL Particle System -- Jonathan Perry" };
...
const int NUM_PARTICLES = 8192; // what needs to be changed
const int LOCAL_SIZE    = 32;
const char *CL_FILE_NAME   = { "jperry.cl" };
...
GLuint			hPobj; // host Points object
GLuint			hCobj;
cl_mem			dPobj;
cl_mem			dCobj;
struct xyzw *	hVel;
...
InitLists( )
{
	...
	glNewList( SphereList, GL_COMPILE );
		glColor3f( .1f, .9f, 0. );
		glPushMatrix( );
			glTranslatef( -550., -800., 0. );  // arg1 how far left the sphere is, arg 2 is how far down the sphere is
			glutWireSphere( 300., 100, 100 ); // arg 1 is the diameter size
		glPopMatrix( );
	glEndList( );
