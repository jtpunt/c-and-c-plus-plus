...
main( int argc, char *argv[ ] )
{
	...
	// c should be read-only memory since we read data from arrays A,B,C to compute the equation A*B+C and store the result in D
	...
	cl_kernel kernel = clCreateKernel( program, "MultAdd", &status );
	if( status != CL_SUCCESS )
